...
import { isAllowedResourceUrl, safeMediaUrl } from './security';
...
export function itemSummary(item: DdysItem): string {
  return String(item.description || item.intro || item.summary || item.note || item.content || item.bio || '').replace(/<[^>]*>/g, '').slice(0, 180);
}
